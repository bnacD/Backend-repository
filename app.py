...
# Basic rate limiter implementation (without flask-limiter dependency)
import time
...
try:
    ...
    print("⚠️ Prophet not available. Using statistical fallback only.")

# METAR parser
try:
    from metar import Metar
    METAR_PARSER_AVAILABLE = True
except ImportError:
    METAR_PARSER_AVAILABLE = False
    print("⚠️ python-metar not available. Using basic parsing.")
...
MAX_YEARS_HISTORICAL = 30
MAX_DATA_POINTS = 10000
DEFAULT_CACHE_TTL_HOURS = 1

# Variable limits for validation
VARIABLE_LIMITS = {
...
# ============ DATA CACHE CLASS ============
class DataCache:
    def __init__(self, cache_dir: str = "./cache", ttl_hours: int = DEFAULT_CACHE_TTL_HOURS):
    ...
    def set(self, key: str, data: Any) -> None:
        """Cache data"""
        if not self.cache_dir or data is None:
            return
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            return jsonify({
                ...
                "details": "lat and lon must be numeric values"
            }), 400
...
def monitor_performance(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            execution_time = time.time() - start_time
            logger.error(f"{f.__name__} failed after {execution_time:.2f}s: {e}")
            raise
    return decorated_function

# ============ UTILITY FUNCTIONS ============
def validate_date_range(start: datetime.date, end: datetime.date) -> Tuple[bool, Optional[str]]:
    """Validate date range"""
    if start > end:
        return False, "Start date must be before end date"
    
    days_diff = (end - start).days
    if days_diff > 365 * 10:  # Max 10 years
        return False, "Date range too large (max 10 years)"
    
    return True, None

def clean_invalid_values(df: pd.DataFrame) -> pd.DataFrame:
    """Clean invalid weather values"""
    if df.empty:
        return df
    ...
    if 'value' in df.columns:
        ...
        # Remove## 🔧 Código Corregido y Mejorado - Parte 1/2

Gracias por la revisión detallada. Aquí está el código corregido con todas las mejoras implementadas:
...
# METAR parsing library (optional but recommended)
try:
...
WEATHERAPI_KEY = os.environ.get("WEATHERAPI_KEY", "")
VISUALCROSSING_KEY = os.environ.get("VISUALCROSSING_KEY", "")
API_SECRET_KEY = os.environ.get("API_SECRET_KEY", "")  # For authentication
...
def check_api_key(api_key: str) -> bool:
    ...
    if not API_SECRET_KEY:  # No authentication required if not set
        return True
...
def rate_limit(limit=10):
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            # Check if authenticated user has higher limits
            auth_header = request.headers.get('Authorization')
            if auth_header and API_SECRET_KEY:
                limit_multiplier = 5  # Authenticated users get 5x the limit
            else:
                limit_multiplier = 1
            
            if not rate_limiter.is_allowed(client_id, limit * limit_multiplier):
                return jsonify({
                    "error": "Rate limit exceeded",
                    "retry_after": 60
                }), 429
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            return jsonify({
                ...
                "required": ["lat", "lon"],
                "example": "/api/forecast?lat=-34.5&lon=-58.4&type=real"
            }), 400
...
def monitor_performance(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if execution_time > 5:  # Log slow requests
            logger.warning(f"{f.__name__} slow execution: {execution_time:.2f}s")
        else:
            logger.info(f"{f.__name__} executed in {execution_time:.2f}s")
...
def parse_metar_advanced(metar_data: str) -> Dict[str, Any]:
    """Advanced METAR parsing using python-metar library"""
    if METAR_PARSER_AVAILABLE:
        try:
            obs = Metar(metar_data)
            result = {}
            
            if obs.temp:
                result["temperatura_C"] = obs.temp.value()
            if obs.dewpt:
                result["dewpoint_C"] = obs.dewpt.value()
            if obs.wind_speed:
                result["viento_mps"] = obs.wind_speed.value() * 0.514444
            if obs.press:
                result["presion_mb"] = obs.press.value()
                
            # Calculate humidity if temp and dewpoint available
            if "temperatura_C" in result and "dewpoint_C" in result:
                temp = result["temperatura_C"]
                dew = result["dewpoint_C"]
                vapor_pressure = 6.112 * np.exp((17.67 * dew) / (dew + 243.5))
                saturation_pressure = 6.112 * np.exp((17.67 * temp) / (temp + 243.5))
                humidity = (vapor_pressure / saturation_pressure) * 100
                result["humedad_pct"] = round(min(100, max(0, humidity)), 1)
                
            return result
        except Exception as e:
            logger.warning(f"Advanced METAR parsing failed: {e}")
            return {}
    return {}

def fetch_metar(icao_code: str = "SGAS") -> Optional[Dict[str, Any]]:
    """Fetch METAR weather data with improved parsing"""
    try:
        ...
        advanced_data = parse_metar_advanced(metar_data)
        if advanced_data:
...
def get## 📦 Parte 2/2 - Weather API Complete Code
...
def get_historical_day_average(lat: float, lon: float, target_date: datetime.date, 
    ...
    """Get historical average for a specific day across multiple years"""
...
def simple_prediction(lat: float, lon: float, target_date: datetime.date, 
    ...
    for year_offset in range(1, years_window + 1):
        try:
            ...
            # Search in ±3 days window for better robustness
...
def train_predict_daily_advanced(lat: float, lon: float, target_date: datetime.date, 
                               variable: str, years_window: int = 10) -> Optional[Dict[str, Any]]:
    """Advanced Prophet prediction with dynamic parameter tuning"""
    ...
    try:
        ...
        # Dynamic parameter tuning based on variable type
        prophet_params = {
            "Temperature (°C)": {
